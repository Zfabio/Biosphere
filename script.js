import { getBioToolContent } from "./js/modules/bioToolContent.js";
import { attachToolEventListeners } from "./js/modules/bioToolInteractions.js";
import {
  buildCellMap,
  initModalUI,
  reRenderCurrentCellModal,
  setGlobalUnit,
  l3UnitState
...
(function initGlobalDragTracking() {
  ...
  document.addEventListener('pointermove', (e) => {
    ...
    if (Math.abs(dx) > DRAG_THRESHOLD || Math.abs(dy) > DRAG_THRESHOLD) {
      window._bioIsDragging = true;
    }
  }, true);
  document.addEventListener('pointerup', () => {
    pointerDown = false;
    // Delay clearing drag state so click handlers see it
    setTimeout(() => { window._bioIsDragging = false; }, 80);
  }, true);
...
window._bioAnimPaused = savedAnimationState.paused;
window._bioAnimSpeed = savedAnimationState.speed;
...
function initWelcomeModal() {
  ...
  const lastForced = localStorage.getItem("biosphere_force_refresh");
  if (lastForced !== CURRENT_VERSION) {
    localStorage.setItem("biosphere_force_refresh", CURRENT_VERSION);
  ...
  function closeWelcome() {
    ...
    localStorage.setItem("biosphere_welcomed", "true");
  ...
  function closeChangelog() {
    ...
    document.body.classList.remove("hide-nav");
    localStorage.setItem("biosphere_changelog_seen", CURRENT_VERSION);
    // Also mark as welcomed so the welcome modal won't pop up after
    localStorage.setItem("biosphere_welcomed", "true");
  }

  // ===== Decide which to show =====
  const seenChangelogVersion = localStorage.getItem("biosphere_changelog_seen");
  const hasVisited = localStorage.getItem("biosphere_welcomed");
...
function bootstrapApp() {
  try {
    ...
    const seenOnboardingVersion = localStorage.getItem("biosphere_onboarding_seen_version");
    if (seenOnboardingVersion !== ONBOARDING_VERSION) {
      localStorage.setItem("biosphere_onboarding_seen_version", ONBOARDING_VERSION);
      localStorage.removeItem("biosphere_welcomed_v2");
    }

    if (!localStorage.getItem("biosphere_welcomed_v2")) {
      initOnboardingFlow();
      return;
    }
    ...
    if (tableContainer) buildCellMap(tableContainer);
    initModalUI();

    const toolsModalController = createToolsModalController({
      getToolContent: getBioToolContent,
    ...
    const pageCtrl = initPageController({
      ...
      onWorksheetPageShown: () => {
        void ensureWorksheetReady().catch((error) =>
          console.error("Worksheet lazy init error:", error),
        );
      },
      onSettingsPageShown: () => {
