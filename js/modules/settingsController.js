...
const ANIMATION_PAUSED_KEY = "biosphere_anim_paused";
const ANIMATION_SPEED_KEY = "biosphere_anim_speed";
...
function initSettingsSuggestionForm() {
  ...
  const sendSuggestion = async () => {
    ...
    await submitSuggestion(text, { source: "biosphere Settings" });
...
function initAnimationControls() {
  ...
  if (speedSlider) {
    speedSlider.value = window._bioAnimSpeed;
    if (speedLabel) {
      speedLabel.textContent = `${window._bioAnimSpeed.toFixed(1)}×`;
    }
  }

  if (playToggle) {
    updatePlayToggleIcon(playToggle, window._bioAnimPaused);
  }

  applyAnimationPauseState(window._bioAnimPaused);

  if (playToggle) {
    playToggle.addEventListener("click", () => {
      window._bioAnimPaused = !window._bioAnimPaused;
      localStorage.setItem(ANIMATION_PAUSED_KEY, window._bioAnimPaused);
      updatePlayToggleIcon(playToggle, window._bioAnimPaused);
      applyAnimationPauseState(window._bioAnimPaused);
    });
  }
...
function initPreferencesCard() {
  ...
  if (reduceToggle) {
    const isReduced = localStorage.getItem("biosphere_reduce_motion") === "true";
    reduceToggle.checked = isReduced;
    
    reduceToggle.addEventListener("change", (e) => {
      localStorage.setItem("biosphere_reduce_motion", e.target.checked);
