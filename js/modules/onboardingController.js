...
/**
 * Enhanced Onboarding Flow for biosphere 2.0
 */
export function initOnboardingFlow() {
  const overlay = document.createElement("div");
  overlay.id = "biosphere-onboarding-overlay";
  overlay.innerHTML = `
    <style>
      #biosphere-onboarding-overlay {
        position: fixed;
        inset: 0;
        background: #ffffff;
        z-index: 1000000;
    ...
    <div id="animation-stage" style="display:flex; flex-direction:column; align-items:center;">
        <div class="logo-container" id="onboarding-logo">
          ...
          <span class="logo-text">biosphere</span>
        </div>
  ...
  function startDanmaku() {
    ...
    if (allPhrases.length === 0) {
        allPhrases.push("BioSphere", "Biology", "Cell", "Organelle", "Life", "Visualization");
    }
  ...
  langScreen.addEventListener("click", (e) => {
    ...
    localStorage.setItem("biosphere_welcomed_v2", "true");
    sessionStorage.setItem("biosphere_lang_transition", "true");
    ...
    requestAnimationFrame(() => {
      ...
      setTimeout(() => {
        localStorage.setItem("biosphere_lang", lang);
