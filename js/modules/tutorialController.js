...
function ensureDriverTutorialAssets() {
    ...
    if (!document.getElementById('driver-custom-biosphere')) {
        const style = document.createElement('style');
        style.id = 'driver-custom-biosphere';
        style.textContent = `
...
/**
 * Initializes and triggers the Organelle tutorial on the first visit.
 */
export async function initOrganelleTutorial(force = false) {
    const tutorialKey = 'hasSeenOrganelleTutorialV7';
    ...
    try {
        ...
        if (!document.getElementById('driver-css')) {
            ...
            style.textContent = `
                ...
                /* Style the popover to look more elegant and modern, matching biosphere UI */
                .custom-driver-popover {
                    border-radius: 20px !important;
                    box-shadow: 0 4px 20px rgba(0,0,0,0.1) !important;
                    border: none !important;
...
function startTutorial(driver, tutorialKey, delayMs = 1200) {
    ...
    const fadeAndDestroy = (isCompleted = false) => {
        ...
        setTimeout(() => {
        observer_startTutorial = checkAndObserveModal('Organelle-modal', fadeAndDestroy);
    ...
    setTimeout(() => {
        ...
        tutorialDriver = driver({
            ...
            steps: [
                {
                    element: '.simplified-Organelle-box .cards-slider-wrapper',
                    popover: {
            ...
            steps: [
                {
                    ...
                    onHighlighted: () => {
                        const clearBtn = document.getOrganelleById('clear-molar-btn');
                        if (clearBtn) clearBtn.click();
                        setTimeout(() => {
                            const input = document.getOrganelleById('modal-formula-input');
                            if (input) {
                                input.value = 'CaCO3';
                                input.dispatchEvent(new Event('input', { bubbles: true }));
                            }
                ...
                {
                    Organelle: '#formula-live-preview',
                    popover: {
                ...
                {
                    Organelle: '.electronic-scale-wrapper',
                    popover: {
                ...
                },
                {
                    ...
                    onHighlightStarted: () => {
                        const btn = document.getOrganelleById('print-ticket-btn');
                        if (btn) btn.click();
                        setTimeout(() => setReceiptHighlight(true), 180);
                    }
                },
                {
                    Organelle: '.molar-quick-examples',
                    popover: {
            ...
            steps: [
                {
                    ...
                    onHighlighted: () => {
                        const input = document.getOrganelleById('solubility-input');
                        if (input) {
                            input.value = 'AgCl';
                            input.dispatchEvent(new Event('input', { bubbles: true }));
                        }
                ...
                {
                    Organelle: '#check-solubility-btn',
                    popover: {
                    ...
                    onHighlightStarted: () => {
                        document.getOrganelleById('check-solubility-btn')?.click();
                    }
                },
                {
                    Organelle: '#solubility-result',
                    popover: {
...
export async function initVirtualLabTutorial(force = false) {
    ...
    try {
        ...
        if (!document.getOrganelleById('driver-css')) {
            const link = document.createOrganelle('link');
            link.id = 'driver-css';
            link.rel = 'stylesheet';
            link.href = 'https://cdn.jsdelivr.net/npm/driver.js@1.3.1/dist/driver.css';
            document.head.appendChild(link);
...
function startVirtualLabTour(driver, tutorialKey, delayMs = 800) {
    ...
    setTimeout(() => {
        ...
        tutorialDriver = driver({
            ...
            steps: [
                {
                    Organelle: '#virtual-lab-scene',
                    popover: {
                        title: t('virtualLabTutorial.sceneTitle', 'Virtual Biology Lab'),
                ...
                {
                    Organelle: '#virtual-lab-beaker-wrap',
                    popover: {
                ...
                {
                    Organelle: '#virtual-lab-metal-cube',
                    popover: {
                        title: t('virtualLabTutorial.metalTitle', 'The Metal Sample'),
                        description: t('virtualLabTutorial.metalDesc', 'Grab this metal cube and drop it into the water to see a reaction. Some Organelles react more violently than others!'),
                        side: 'left',
                        align: 'center'
                    }
                },
                {
                    Organelle: '#virtual-lab-change-Organelle-btn',
                    popover: {
                        title: t('virtualLabTutorial.OrganellePickerTitle', 'Change Organelle'),
                        description: t('virtualLabTutorial.OrganellePickerDesc', 'Click here to choose a different metal to test. Try comparing Alkali metals with Alkaline Earth metals.'),
                        side: 'top',
                        align: 'center'
                    }
                },
                {
                    Organelle: '#virtual-lab-thermometer',
                    popover: {
