...
export function ensureThreeLibLoaded() {
  ...
  threeLoadPromise = new Promise((resolve, reject) => {
    const existing = document.querySelector('script[data-biosphere-three="1"]');
    if (existing) {
    ...
    script.async = true;
    script.dataset.biosphereThree = "1";
...
export function updateAtomStructure(element) {
  ...
  if (atomicNumber > 1) {
    ...
    if (atomicNumber === 11 && window._bioNaMode === "cloud") lightIntensity = 5.0;
...
// ===== Biological Structure Builder (BioSphere) =====
export function updateBioStructure(bioEntry) {
  if (!atomGroup) return;
  while (atomGroup.children.length > 0) {
    atomGroup.remove(atomGroup.children[0]);
  }
  ...
  const entryId = bioEntry.id;
  const shape = bioEntry.level3_properties?.render?.shape || "sphere";
  const mainColor = new THREE.Color(bioEntry.level3_properties?.render?.color || "#10b981");
  const secColor = new THREE.Color(bioEntry.level3_properties?.render?.secondaryColor || "#34d399");

  const bioGroup = new THREE.Group();
  atomGroup.add(bioGroup);

  if (shape === "pored-sphere") {
    // Nucleus style
    const geo = getSphereGeometry(5, 64);
    ...
    // Add "pores" - small dark spheres on surface
    const poreGeo = getSphereGeometry(0.4, 16);
    const poreMat = new THREE.MeshBasicMaterial({ color: 0x111111 });
    for (let i = 0; i < 40; i++) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const tubeMat = new THREE.MeshBasicMaterial({ color: secColor, transparent: true, opacity: 0.6 });
        chromatinGroup.add(new THREE.Mesh(tubeGeo, tubeMat));
    }
    bioGroup.add(chromatinGroup);
    _nucleusGroupRef = bioGroup;
    ...
    const buildStrand = (offset) => {
        const points = [];
        for (let i = 0; i < pointsPerTurn * turns; i++) {
            const t = i / pointsPerTurn;
            const angle = t * Math.PI * 2 + offset;
            const y = (i / (pointsPerTurn * turns)) * height - height/2;
            points.push(new THREE.Vector3(radius * Math.cos(angle), y, radius * Math.sin(angle)));
        }
        const curve = new THREE.CatmullRomCurve3(points);
        const tubeGeo = new THREE.TubeGeometry(curve, 128, 0.2, 8, false);
        const tubeMat = new THREE.MeshStandardMaterial({ color: mainColor, metalness: 0.5, roughness: 0.2 });
        return new THREE.Mesh(tubeGeo, tubeMat);
    };

    const strand1 = buildStrand(0);
    const strand2 = buildStrand(Math.PI);
    helixGroup.add(strand1, strand2);

    // Bases (ladder rungs)
    const rungGeo = new THREE.CylinderGeometry(0.1, 0.1, radius * 2);
    const rungMat = new THREE.MeshStandardMaterial({ color: secColor });
    for (let i = 0; i < pointsPerTurn * turns; i += 5) {
        const t = i / pointsPerTurn;
    ...
    bioGroup.add(helixGroup);
    _wobbleGroupRef = helixGroup;

  } else if (shape === "folded-bean") {
    // Mitochondria
    const beanGeo = new THREE.CapsuleGeometry(3, 4, 32, 64);
    ...
    const outer = new THREE.Mesh(beanGeo, beanMat);
    bioGroup.add(outer);
    
    // Internal cristae (nested smaller capsules with "folds")
    const innerGeo = new THREE.CapsuleGeometry(2.5, 3.5, 32, 64);
    const innerMat = new THREE.MeshStandardMaterial({ color: secColor, metalness: 0.3, roughness: 0.4 });
    const inner = new THREE.Mesh(innerGeo, innerMat);
    bioGroup.add(inner);
    
    _nucleusGroupRef = bioGroup;
  } else {
    // Default fallback: Stylized Cell Sphere
    const geo = getSphereGeometry(5, 32);
    const mat = new THREE.MeshStandardMaterial({ color: mainColor });
    bioGroup.add(new THREE.Mesh(geo, mat));
  }

  atomGroup.userData.maxRadius = 8;
  atomGroup.userData.popStartTime = Date.now();
  atomGroup.scale.set(0.1, 0.1, 0.1);
}
...
export function animateAtom() {
  ...
  const isPaused = window._bioAnimPaused || false;
  const speedMul = (typeof window._bioAnimSpeed === "number") ? window._bioAnimSpeed : 0.6;
  ...
  const isEdu = isNa && window._bioNaMode === "educational";
