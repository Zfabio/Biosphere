// UI Controller - Cell Map Grid & Bio-Modal
...
// v2 dataset stub — file removed; branches gated by biosphereVersion === 'new' are inert.
...
function createLegend(container) {
  ...
  const categories = [
    { name: "Organelle", class: "organelle" },
    { name: "Biomolecule", class: "biomolecule" },
    { name: "Genetics", class: "genetics" }
  ];
  ...
  categories.forEach((cat) => {
    ...
    item.addEventListener("mouseenter", () => highlightCategory(container, cat.class));
    item.addEventListener("mouseleave", () => clearHighlights(container));
...
function updatePeriodicTableLocalizedText(tableContainer) {
  // BioSphere specialized localization (subset for now)
  tableContainer.querySelectorAll(".element.bio-cell").forEach((cell) => {
    const bioId = cell.dataset.elementNumber;
    const entry = bioData[bioId];
    if (!entry) return;
    
    const nameEl = cell.querySelector(".name");
    if (nameEl) nameEl.textContent = entry.name;
    
    cell.setAttribute("aria-label", `${entry.name} (${entry.symbol})`);
  });
}

// ===== Biological Grid Generation =====
export function buildCellMap(tableContainer) {
  try {
    eitController.resetEITRegistry();
    eitController.resetEITState();

    tableContainer.classList.add("bio-grid");
    // Clean container
    tableContainer.innerHTML = "";
    
    const bioEntries = Object.values(bioData);
    if (!bioEntries || bioEntries.length === 0) {
      console.warn("BioSphere: No bioData found to render.");
      return;
    }
    ...
    bioEntries.forEach((entry) => {
      try {
        const r = entry.row;
        const c = entry.column + 7; // Center it in the 18-col grid
        const cell = document.createElement("div");
        cell.classList.add("element", "bio-cell");
        cell.dataset.elementNumber = String(entry.id);
        cell.setAttribute("role", "button");
        cell.setAttribute("tabindex", "0");
        
        const catClass = (entry.category || "unknown").toLowerCase();
        cell.classList.add(catClass);
        
        // Use symbol for the number slot as it's more appropriate for biology
        cell.innerHTML = `
            <span class="number">${entry.symbol}</span>
            <span class="symbol">${(entry.name || 'B').charAt(0)}</span>
            <span class="name">${entry.name || 'Bio Entry'}</span>
        `;
        
        const openBioModal = () => showModal({ number: entry.id });
        cell.addEventListener("click", openBioModal);
        bindKeyboardActivation(cell, openBioModal);
        
        cell.style.gridRow = (r || 0) + 1; // Row offset for EIT
        cell.style.gridColumn = c || 1;
        tableContainer.appendChild(cell);
      } catch (entryError) {
        console.error("BioSphere: Failed to render individual entry:", entry, entryError);
      }
    });
    ...
    console.error("BioSphere: Critical error building Periodic Table Grid:", globalError);
    tableContainer.innerHTML = `<div class="error-placeholder">Failed to load Cell Map. Please refresh.</div>`;
  }
}
...
// ===== Pure Helpers =====
export function reRenderCurrentCellModal() {
  if (window.currentCellEntry) {
    ...
    showModal(window.currentCellEntry);
  }
}
...
let savedUnits = null;
try {
  const stored = localStorage.getItem('biosphere_units');
...
function saveUnits() {
  try {
    localStorage.setItem('biosphere_units', JSON.stringify(l3UnitState));
  } catch (e) {
...
function populateBioView(bioEntry) {
  ...
  // Header
  setText("#headline-symbol", bioEntry.symbol || "Bio");
  setText("#headline-name", bioEntry.name || "Entry");
  setText("#headline-atomic", bioEntry.id);
  setText("#headline-mass", bioEntry.type || "Organelle");
  ...
  if (formationText) {
    formationText.textContent = l1.description || l1.summary || "";
  }

  // Level 2: Molecular (Yellow)
  const l2 = bioEntry.level2_molecular || {};
  const yellowCard = document.getElementById("ion-l2-standard");
  if (yellowCard) {
    // Repurpose labels for bio
    const rows = yellowCard.querySelectorAll(".info-row");
    if (rows[0]) {
      rows[0].querySelector(".info-label").textContent = "Composition";
      rows[0].querySelector(".info-value").textContent = l2.composition || "---";
    }
    if (rows[1]) {
      rows[1].querySelector(".info-label").textContent = "Avg. Size";
      rows[1].querySelector(".info-value").textContent = l2.size || "---";
    }
    if (rows[2]) {
      rows[2].querySelector(".info-label").textContent = "ID Code";
      rows[2].querySelector(".info-value").textContent = `#${bioEntry.id}`;
    }

    const isoSection = yellowCard.querySelector(".ions-section");
    if (isoSection) {
      isoSection.querySelector(".info-label").textContent = "Sub-components";
      const isoList = document.getElementById("ion-l2-isotopes");
      if (isoList && l2.components) {
        isoList.innerHTML = l2.components.map(c => `<div class="ion-item" style="justify-content: center;"><span>${c}</span></div>`).join("");
      }
    }
  }

  // Level 3: Properties (Blue)
  const l3 = bioEntry.level3_properties || {};
  const blueCard = document.querySelector(".blue-rectangle .card-info-container");
  if (blueCard) {
    const configHero = document.getElementById("ion-l3-config");
    if (configHero) configHero.textContent = l3.render?.shape || "Complex";
    
    const oxHeader = blueCard.querySelector(".l3-stat-label");
    if (oxHeader) oxHeader.textContent = "Activity Level";
    
    const oxCont = document.getElementById("ion-l3-oxidation");
    if (oxCont) oxCont.innerHTML = `<div class="ox-pill common">${Math.round((l3.render?.complexity || 0.5) * 100)}%</div>`;

    const statValues = blueCard.querySelectorAll(".l3-stat-value");
    const statLabels = blueCard.querySelectorAll(".l3-stat-label");
    if (statLabels[1]) statLabels[1].textContent = "Color Code";
    if (statValues[2]) statValues[2].textContent = l3.render?.color || "#ffffff";
    if (statLabels[2]) statLabels[2].textContent = "Primary Color";
    if (statValues[3]) statValues[3].textContent = l3.render?.secondaryColor || "#ffffff";
  }

  // Level 4: History (Red)
  const l4 = bioEntry.level4_history_stse || {};
  const redCard = document.getElementById("ion-l4-container");
  if (redCard) {
    setText("#ion-l4-year", l4.history?.discoveryYear || "Unknown");
    const uses = document.getElementById("ion-l4-uses");
    if (uses) uses.textContent = (l4.commonUses || []).join(", ");
    
    // Repurpose Hazards to "Discovered By" for bio
    const hazLabel = redCard.querySelector("span[style*='color: #DC2626']");
    const hazValContainer = document.getElementById("ion-l4-hazards");
    if (hazLabel) {
      hazLabel.textContent = "Discovered By";
      hazLabel.style.color = "#059669";
    }
    if (hazValContainer) {
      hazValContainer.textContent = l4.history?.discoveredBy || "---";
      hazValContainer.style.color = "#064E3B";
      hazValContainer.parentElement.style.background = "rgba(16, 185, 129, 0.1)";
      hazValContainer.parentElement.style.borderColor = "rgba(16, 185, 129, 0.2)";
    }
  }
...
function populateSimplifiedView(element) {
  const bioEntry = bioData[element.number];
  if (bioEntry) {
    populateBioView(bioEntry);
    return;
  }
  ...
  if (greenCard) {
    ...
    if (window.biosphereVersion === 'new' && v2Data) {
  ...
  if (yellowCard) {
    ...
    if (window.biosphereVersion === 'new' && v2Data && v2Data.level2_atomic.mass.standard) {
    ...
    if (window.biosphereVersion === 'new' && v2Data && v2Data.level3_properties?.electronic?.configuration) {
  ...
  if (redCard) {
    ...
    if (window.biosphereVersion === 'new' && v2Data && v2Data.level4_history_stse.history.discoveryYear) {
...
export function showModal(element) {
  ...
  window.currentCellEntry = element;
  clearHeadlineResizeHandler();
  const bioEntry = bioData[element.number];
  const finallyElementData = (finallyData && finallyData[element.number]) || {};
  ...
  if (headlineName) {
    const lang = getLang();
    let displayHeadline = bioEntry ? bioEntry.name : localizeElementName(element);
    const annotation = bioEntry ? "" : getElementAnnotation(element.number, lang);
    if (annotation) {
  ...
  if (modalDiscovery && !bioEntry) modalDiscovery.textContent = element.discovery;
  if (modalEtymology && !bioEntry) modalEtymology.textContent = element.etymology;
  if (modalDescription) modalDescription.textContent = element.description;
  
  if (bioEntry) {
    // BioSphere specialized modal updates
    const modalLoc = document.getElementById("el-modal-l1-phase");
    if (modalLoc) modalLoc.textContent = bioEntry.level1_basic.location;
    const modalFunc = document.getElementById("el-modal-l1-category");
    if (modalFunc) modalFunc.textContent = bioEntry.level1_basic.function;
    
    // 3D Rendering
    void ensureThreeLibLoaded().then(() => {
        updateBioStructure(bioEntry);
        reset3DView();
    });
  } else {
    // Bio 3D Rendering (Fallback if not already handled)
    void ensureThreeLibLoaded().then(() => {
        updateBioStructure(bioEntry || element);
        reset3DView();
    });
  }
  ...
  if (eduCardsContainer) {
    ...
    if (hasAdvanced) {
      ...
      if (eduData.components) {
        advancedHtml += `
                      <div class="data-section">
                          <div class="data-title">Key Sub-components</div>
                          <div class="bio-components-list" style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px;">
                              ${eduData.components
                                .map(
                                  (c) => `
                                  <span class="bio-component-tag" style="background: rgba(16, 185, 129, 0.1); color: #065f46; padding: 6px 12px; border-radius: 9999px; font-weight: 600; font-size: 0.85rem; border: 1px solid rgba(16, 185, 129, 0.2);">${c}</span>
                                `,
                                )
                                .join("")}
                          </div>
